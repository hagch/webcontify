drop table if exists WEBCONTIFY_VIEW_RELATION CASCADE;
drop table if exists WEBCONTIFY_VIEW CASCADE;
...
drop type if exists WEBCONTIFY_VIEW_AGGREGATION_TYPE CASCADE;
...
create type WEBCONTIFY_VIEW_AGGREGATION_TYPE as enum (
    'OBJECT',
    'LIST'
);

create table WEBCONTIFY_VIEW_RELATION(
                                         ID BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
                                         VIEW_ID BIGINT REFERENCES WEBCONTIFY_VIEW(ID) ON DELETE CASCADE,
                                         COLLECTION_ID BIGINT REFERENCES WEBCONTIFY_COLLECTION(ID) ON DELETE CASCADE,
                                         RELATION_ID BIGINT REFERENCES WEBCONTIFY_COLLECTION_RELATION(ID) ON DELETE CASCADE,
                                         AGGREGATION_TYPE WEBCONTIFY_VIEW_AGGREGATION_TYPE NOT NULL,
                                         VIEW_RELATION_PARENT_ID BIGINT,
                                         CONSTRAINT FK_PARENT_VIEW_RELATION FOREIGN KEY(VIEW_RELATION_PARENT_ID) REFERENCES WEBCONTIFY_VIEW_RELATION(ID) ON DELETE CASCADE
);