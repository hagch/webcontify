...
@Component
class CollectionItemDao(val dslContext: DSLContext, val columnHandlerStrategy: ColumnHandlerStrategy) {

  fun getItemByIdFor(collection: WebContifyCollectionDto, identifierMap: Map<String, String?>): Map<String, Any> {
    val columnTypeMap = getColumnTypeMap(collection)
    val identifierTypeMap =  identifierMap.entries.associate{ it.key to Pair(it.value,
      columnTypeMap[it.key]
    ) }
    val fields =
  ...
  fun create(collection: WebContifyCollectionDto, item: Map<String,Any>): Map<String,Any> {
    //TODO primary key generation
    val columnTypeMap = getColumnTypeMap(collection)
    val identifierTypeMap =  item.entries.associate{ it.key to Pair(it.value,
      columnTypeMap[it.key]
    ) }
    val fields = item.keys.map { field("\"${it}\"") }
    val values = identifierTypeMap.map {
      if(it.value.second == null){
        it.value
      } else {
  ...
  private fun getColumnTypeMap(collection: WebContifyCollectionDto): Map<String, DataType<*>> {
    return collection.columns?.associateBy({ it.name }, { columnHandlerStrategy.getHandlerFor(it.type).getColumnType() }) ?: mapOf()
  }
}
